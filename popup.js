...
function confirmPassphrase() {
  ...
  if (passphraseInput.trim()) {
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      if (tabs.length > 0) {
        chrome.tabs.sendMessage(tabs[0].id, { action: 'setPassphrase', passphrase: passphraseInput }, (response) => {
...
function resetPassphrase() {
  chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
    if (tabs.length > 0) {
      chrome.tabs.sendMessage(tabs[0].id, { action: 'resetPassphrase' }, (response) => {
        if (response && response.status === "success") {
...
chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
  if (tabs.length > 0) {
    chrome.tabs.sendMessage(tabs[0].id, { action: 'checkPassphrase' }, (response) => {
